...
const handleCloseFunnelPopup = () => {
  ...
  $('select[name="consultor-campo"]').val("");
}
...
const handleLoadLeadInfo = (url) =>{
  $('select[name="consultor-campo"]').val("");
  $('select[name="consultor-campo"] option').remove();
  $.ajax({
    type: "GET",
    url: url,
    timeout: 5000,
    ...
    success: function(data) {
      ...
      $.ajax({
        type: "GET",
        url: 'http://moraremfranca.com.br/admin/public/consultores',
        contentType: "application/json; charset=utf-8",
        cache: false,
        beforeSend: function() {
          $('#consultor-campo').append('<option value="">Carregando...</option>');
        },
        success: function(dataConsultor) {
          $('select[name="consultor-campo"] option').remove();
          $('#consultor-campo').append('<option value="">Selecione o consultor</option>');
          for (var index = 0; index <= dataConsultor.length; index++) {
            console.log(dataConsultor[index])
            if(dataConsultor[index]!='undefined'){
              if(dataConsultor[index].id==data.consultores_id){
                $('#consultor-campo').append('<option value="' + dataConsultor[index].id + '" selected>' + dataConsultor[index].name + '</option>');
              }else{
                $('#consultor-campo').append('<option value="' + dataConsultor[index].id + '">' + dataConsultor[index].name + '</option>');
              }
            }
          }
        }
