...
<div class="funnel__container--mobile">
  @foreach ($funnels as $f)
  <div class="connected-box--mobile">
    <div class="connected-title--mobile">
      {{$f->name}} <i class="far fa-caret-square-down"></i>
    </div>

    <div class="conected-content--mobile">
      @foreach ($f->leads as $l)
      <a title="" href="{{route('lead.edit', ['id' => $l->id])}}" class="client-mobile">
        <div>
          <p class="client-mobile__nome">{{$l->name}}</p>
          <p class="client-mobile__infos">{{$l->phone}}</p>
          <p class="client-mobile__infos"><i class="far fa-clock"></i>  {{ \Carbon\Carbon::parse($l->created_at)->format('d/m/Y \à\s H:i')}}</p>
        </div>
        <div class="client-mobile__ver">VER <i class="far fa-caret-square-right"></i></div>
      </a>
      @endforeach
    </div>
    ...
    <div class="funnel-popup__content">
      ...
      <div class="funnel-popup__lead-info">
        ...
        <div class="funnel-popup__lead-info__content">
          ...
          <p><b>Empreendimento:</b><span class="message" id="lead-empreendimento"></span></p>
          <p><b>Origem:</b><span class="message" id="lead-origin"></span></p>
          <p>
            <b>Consultor de campo:</b>
            <select name="consultor-campo" class="form-control" id="consultor-campo">
...
<script>
  ...
  $('select[name="consultor-campo"]').change(function(){
    const lead_id = $('#lead_id').val();
    const consultor = $(this).val();
    const dataToSend = {
      lead_id,
      consultor,
    };
    $.ajax({
      type: "POST",
      url: '{{route('lead.updateConsultor')}}'+'?_token=' + '{{ csrf_token() }}',
      data: dataToSend,
